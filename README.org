...
#+caption: Search Notes — latest notes with tags and dates
[[file:src/__tests__/screenshots/baseline/search-notes-latest.png]]

#+caption: Open Note in Emacs — frame reuse for instant open
[[file:src/__tests__/screenshots/baseline/emacs-open-note.png]]
...
#+caption: Unit test run (vitest)
[[file:src/__tests__/screenshots/test-run.gif]]

*** Visual E2E Tests

Automated screenshot capture of Raycast + Emacs interaction with baseline comparison via =pixelmatch=:

#+begin_src sh
npm run test:visual
#+end_src

Requires =ray develop= running, Emacs daemon, and Screen Recording permission for the terminal.

*** Full Suite

#+begin_src sh
npm test          # all tests
npm run test:record  # record unit test GIF via VHS
#+end_src
